...
def plot_fov_when_flare_observed(flare_positions, observation_type, shape=(2200, 2200),
    ...
    for flare_class in class_list:
        ...
        for i in range(0, len(x)):
            fov_mask = fm.flare_mission_sim.fov_coverage_mask([x[i], y[i]], fov, shape)
            heatmap += fov_mask
...
def plot_heatmaps_and_summaries(heatmap, filename, title, figsize=(8, 8), left=0.1, width=0.6,
    ...
    extent = [-image_half_length, image_half_length, -image_half_length, image_half_length]
    im = ax_heatmap.imshow(heatmap/heatmap.max(), cmap=cm.Oranges, origin='lower', extent=extent)
    ...
    ax_heatmap.set_xlabel('solar X (longitude, arcsec)')
    ax_heatmap.set_ylabel('solar Y (latitude, arcsec)')
    ...
    cb.ax.set_ylabel('{:s}/max({:s})'.format(heatmap_map, heatmap_map))
    ...
    ax_longitude.set_ylabel(ylabel)
    ...
    ax_longitude.text(50, 0.1, ratio)
    ax_longitude.set_title('normalized heat as a function of longitude (summed over all latitudes)')
